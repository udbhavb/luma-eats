...
3. **The decision deadline** — optional countdown; when it expires the server auto-locks the top-voted time (ties broken randomly — tied leaders are equally good), then the top-voted place (ties broken by rating). It never auto-picks a zero-vote option, and it locks the time before the place, honoring the when-before-where rule. The sweep runs server-side every 10s, so the decision lands even if nobody has the tab open.
